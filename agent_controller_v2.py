...
#==============================================================================
# SECTION 0: GIT SYNCHRONIZATION AND SAFETY CHECKS
#==============================================================================

def sync_with_remote_and_prepare() -> bool:
    """
    Ensures the local repo is clean and synced with the remote 'main' branch.
    This is the most critical step for reliable automation.
    """
    print("---")
    ...
    try:
        ...
        print("  - This can happen if there is a merge conflict during 'git pull'.")
        print("  - Please resolve any issues manually and then run the script again.")
        print(f"  - Error details: {e.stderr}")
        # Attempt to pop any changes we stashed
        subprocess.run(["git", "stash", "pop"])
        return False

#==============================================================================
# SECTION 1: PARSING AND JULES INTERACTION (Unchanged)
#==============================================================================
...
#==============================================================================
# SECTION 3: MAIN WORKFLOWS (MODIFIED)
#==============================================================================

def run_create_mode():
    """Parses TODO, finds new tasks, and sends them to Jules."""
    print("Running in CREATE mode...")
    if not sync_with_remote_and_prepare(): return # <-- ADDED SAFETY CHECK
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] not in ['create', 'review']:
        ...
        sys.exit(1)
    mode = sys.argv[1]
    if mode == "create": run_create_mode()
    elif mode == "review": run_review_mode()
    print("\nScript finished.")